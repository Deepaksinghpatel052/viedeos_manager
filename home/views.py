...
def get_last_uploaded_vido_by(dummt_data):
	...
	return get_videp.Created_By.name[0:5]+"..."
...
class AboutView(generic.TemplateView):
	...
	def get_context_data(self, **kwargs):
		context = super().get_context_data(**kwargs)
		page_content = None
		if VsCmsPage.objects.filter(keywork='about-us').exists():
			page_content =VsCmsPage.objects.filter(keywork='about-us').first()
		context['page_content'] = page_content
		return context

class TermsUseView(generic.TemplateView):
	template_name = "web/home/cms_pagge.html"
	
	def get_context_data(self, **kwargs):
		context = super().get_context_data(**kwargs)
		page_content = None
		if VsCmsPage.objects.filter(keywork='terms-of-use').exists():
			page_content =VsCmsPage.objects.filter(keywork='terms-of-use').first()
		context['page_content'] = page_content
		return context

class PrivacypPolicyView(generic.TemplateView):
	template_name = "web/home/cms_pagge.html"
	
	def get_context_data(self, **kwargs):
		context = super().get_context_data(**kwargs)
		page_content = None
		if VsCmsPage.objects.filter(keywork='privacy-policy').exists():
			page_content =VsCmsPage.objects.filter(keywork='privacy-policy').first()
		context['page_content'] = page_content
		return context
...
class CareerView(generic.TemplateView):
	...
	def get_context_data(self, **kwargs):
		context = super().get_context_data(**kwargs)
		page_content = None
		if VsCmsPage.objects.filter(keywork='career').exists():
			page_content =VsCmsPage.objects.filter(keywork='career').first()
		context['page_content'] = page_content
		return context
